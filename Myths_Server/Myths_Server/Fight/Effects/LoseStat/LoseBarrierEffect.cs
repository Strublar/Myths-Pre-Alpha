...
namespace Myths_Server
{
    class LoseBarrierEffect : Effect
    {
        ...
        public LoseBarrierEffect(TargetSelector sources, TargetSelector targets, List<int> values) 
        ...
        public override void ExecuteOnTarget(int targetId, Context context, FightHandler fightHandler)
        {
            ...
            Console.WriteLine( fightHandler.Entities[targetId].Definition.Name+" loses "+ values[0] + " barrier");

            Entity target = fightHandler.Entities[targetId];
            fightHandler.FireEvent(new EntityStatChangedEvent(targetId, targetId,
                Stat.barrier, target.GetStat(Stat.barrier) - values[0]));
            ...
            if (values.Count > 1)
            {
                if (values[1] == 1)
                {
                    List<Effect> otherEffect = new List<Effect>{
                        new GainBarrierEffect(new EffectHolderSelector(), new EffectHolderSelector(),
