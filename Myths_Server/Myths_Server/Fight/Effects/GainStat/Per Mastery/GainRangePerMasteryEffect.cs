...
{
    class GainRangePerMasteryEffect : Effect
    {
        ...
        #region Constructor
        public GainRangePerMasteryEffect() : base()
        {
        }

        public GainRangePerMasteryEffect(TargetSelector sources, TargetSelector targets, List<int> values) 
        ...
        public override void ExecuteOnTarget(int targetId, Context context, FightHandler fightHandler)
        {
            ...
            Effect newEffect = new GainRangeEffect(
                sources, targets, new List<int> { computedValue, isTemp });
            newEffect.ExecuteOnTarget(targetId, context, fightHandler);

            /*Console.WriteLine( fightHandler.Entities[targetId].Definition.Name+" Gains "+computedValue+" mobility");

            Entity target = fightHandler.Entities[targetId];
            fightHandler.FireEvent(new EntityStatChangedEvent(targetId, targetId,
                Stat.range, target.GetStat(Stat.range) + computedValue));*/
